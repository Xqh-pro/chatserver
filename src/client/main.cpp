    ...
    for(;;)
    {
        ...
        switch(choice)
        {
            case 1://login业务
            {
                ...
                {
                    char buffer[1024] = {0};
                    len = recv(clientfd,buffer,1024,0);//阻塞等待接受，因为不管成功登录与否都会返回响应消息
                    if(-1==len)
                    {
                    ...
                    else
                    {
                        ...
                        else //登录成功
                        {
                            ...
                            //登录成功， 启动接受线程负责接受数据  该线程在客户端只需要启动一次就行了，避免多次登录造成多线程开启
                            static int readthreadnumber = 0;
                            if(readthreadnumber == 0)  //只会在客户端第一次登录成功时开启接受服务器消息的子线程
                            {
                            ...
                            isMainMenuRunning = true;//进入主菜单交互页面
                            mainMenu(clientfd);
            ...
            case 2://注册业务
            {
                ...
                {
                    char buffer[1024] = {0};
                    len = recv(clientfd,buffer,1024,0);//阻塞等待接受  因为注册请求会有一个响应
                    if(-1==len)
                    {
                    ...
                    {
                        json responsejs = json::parse(buffer);//将接受到的json字符串反序列化为json对象
